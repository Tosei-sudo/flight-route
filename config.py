...
# ── 燃料・バッテリー ──────────────────────────────────────────────────────────
# 推進フェーズ: fuel > 0  → 通常スラスト
# 滑空フェーズ: fuel = 0, battery > 0  → 翼のみで操舵（権限制限あり）
# 弾道フェーズ: fuel = 0, battery = 0  → 無制御、慣性＋抗力のみ
FUEL_CAPACITY      = 100.0  # 燃料容量 [kg]（任意単位）
FUEL_BURN_RATE     = 0.010  # 燃料消費 [kg / (m/s² · s)]（スラスト比例）
BATTERY_CAPACITY   = 30.0   # バッテリー容量 [Wh]（任意単位）
BATTERY_DRAIN_RATE = 0.08   # バッテリー消費 [Wh/s]（滑空制御中）
GLIDE_STEER_LIMIT  = 0.35   # 滑空時の最大操舵加速度割合（max_accel に対する比）
